...
void McsGetLotLoc(void) {
  ...
  strncpy(subStr, lotId, 1);

//  printf("LotId=%s, compare value=%d", lotId,strncmp(subStr, "2",1));

  if (strncmp(subStr, "2",1) == 0 || strncmp(subStr, "3",1) == 0 || strncmp(subStr, "6",1) == 0) {
    //printf(", First Char in Lot:%s \n",  subStr);
    PRS_DEL_InitCommand("MCS_NOT_FOUND", "Lot is outside TOSHIBA MCS stocker");
  } else {
    PRS_DEL_InitCommand("MCS_SUCCESS", config.successfreetext);
    sprintf(tStr, "E%05.5dC3", config.eqptid++);
    PRS_DEL_PutToken("%s", tStr);
    sprintf(tStr, "L%05.5dC3", config.locid++);
    PRS_DEL_PutToken("%s", tStr);
  }

  if (TRC_ChkAlarm())
    TLK_DMQ_ServerPut(hPRIMARY);
  if (config.eqptid > 99999)
    config.eqptid = 1;
  if (config.locid > 99999)
    config.eqptid = 1;
}
...
void McsGetLotAll(void) {
  ...
  for (ictr = 0; totlots > 0 && (TRC_ChkAlarm()); --totlots, ++ictr) {
    ...
    strncpy(subStr, LotList[ictr], 1);
    printf("LotId=%s, First Char in Lot%s:", LotList[ictr], subStr);

    if (strcmp(subStr, "2") == 0 || strcmp(subStr, "3") == 0 || strcmp(subStr, "6") == 0) {
      PRS_DEL_PutToken("%s", "        ");
      PRS_DEL_PutToken("%s", "        ");
    } else {
      sprintf(tStr, "E%05.5dC3", config.eqptid++);
